...
void loop() {
  ...
  if(reversevariable == 0) {
    fwd(255);
    if((distances[left] < 290) && (distances[left] < distances[right])) {
      right_turn();
    }
    else if((distances[right] < 290) && (distances[right] < distances[left])) {
      left_turn();
    } else {
      straight_ahead();
    }
  ...
  if((distances[straight] < 80) || (reversevariable == 1)) {
    reversevariable = 1;
    straight_ahead();
    reverse(200);
    if(distances[straight] > 300) {
      reversevariable = 0;
    }
  }
}
