...
void loop() {
  ...
  if(reverse_variable == 0) {
    fwd(current_speed);
    active_turn = 0;
    if((distances[left] < 340) && (distances[left] < distances[right])) {
      if(distances[left] < 220) {
        current_speed = (top_speed - ((300 - distances[left]) - 30));
        active_right = 1;
        active_turn = 1;
        right_turn();
      }
        ...
        active_turn = 1;
        current_speed = (top_speed - ((300 - distances[left]) - 30));
        specific_turn((1940 - distances[left]) + 50);
      }
    }
    else if((distances[right] < 340) && (distances[right] < distances[left])) {
      if(distances[right] < 220) {
        current_speed = (top_speed - ((300 - distances[left]) - 30));
        active_left = 1;
        active_turn = 1;
        left_turn();
        ...
        active_turn = 1;
        current_speed = (top_speed - ((300 - distances[right]) - 30));
        specific_turn((800 + distances[right]) - 50);
      }
    }
    if(active_turn == 0) {
      straight_ahead();
